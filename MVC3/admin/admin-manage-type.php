...
$query_search=mysqli_query($connection,"SELECT users.FirstName,users.LastName,type.Type_Name,type.Description,type.CreatedDate,type.IsActive,type.ID FROM type LEFT JOIN users ON type.CreatedBy=users.ID WHERE users.FirstName LIKE '%$search_result%' OR users.LastName LIKE '%$search_result%' OR type.Type_Name LIKE '%$search_result%' OR type.Description LIKE '%$search_result%' OR type.CreatedDate LIKE '%$search_result%' OR type.IsActive LIKE '%$search_result%' ORDER BY type.CreatedDate DESC");
if(!$query_search){
	mysqli_error($connection);
}
}
else{
	$query_search=mysqli_query($connection,"SELECT users.FirstName,users.LastName,type.Type_Name,type.Description,type.CreatedDate,type.IsActive,type.ID FROM type LEFT JOIN users ON type.CreatedBy=users.ID   ORDER BY type.CreatedDate DESC");
if(!$query_search){
	mysqli_error($connection);
}
}
...
if(isset($_GET['delete_id'])){
	...
	$query_delete=mysqli_query($connection,"UPDATE type SET IsActive=0 WHERE ID=$deleteid");
	if(!$query_delete){
		mysqli_error($connection);
	}
	header("Location:admin-manage-type.php");
}
	...
	<section id="in-progress-notes" class="admin-manage">
		<div class="container">
			...
			<div class="row no-gutters">
				...
				<div class="col-md-6"><button type="submit" class="btn btn-primary btn-admin  dashboard-btn" onclick="window.location.href='admin-add-type.php'">ADD Type</button></div>
				<form action="admin-manage-type.php" method="post"><div class="dashboard-search">
...
$active="No";
					...
					<td>$fname $lname</td>
					<td>$active</td>
					<td><a href='admin-add-type.php?type_id=$type_id' >
					<img src='images/images/edit.png' ></a>
					<a onclick='Delete_type(this);return false;' href='admin-manage-type.php?delete_id=$type_id'><img src='images/images/delete.png'></a></td>
					";
				}?>
...
var rejectedNotesTable = $('#type-table').DataTable({
    "order": [[ 4, "desc" ]],
