...
if(isset($_POST['submit_review'])){
	echo "submit_review";
	 ...
	 echo $unpublish_id=$_POST['noteid_for_report'];
...
while($row=mysqli_fetch_assoc($query_seller)){
	...
	$downloader_Entry=mysqli_query($connection,"UPDATE downloads SET AttachmentDownloadedDate=NOW(),IsAttachmentDownloaded=1,ModifiedDate=Now(),ModifiedBy=$admin_id WHERE NoteID=$noteid AND DownloaderID=$admin_id AND IsSellerHasAllowedDownload=1");
		...
		$downloader_Entry=mysqli_query($connection,"INSERT INTO downloads (SellerID,DownloaderID,NoteID,IsSellerHasAllowedDownload,AttachmentPath,IsAttachmentDownloaded,AttachmentDownloadedDate,IsPaid,PurchasedPrice,NoteTitle,NoteCategory,CreatedDate,CreatedBy,IsActive) VALUES 
		($sellerID,$admin_id,$noteid,1,'$path',1,NOW(),$Is_Paid,$prince,'$title','$category',NOW(),$admin_id,1)");
	}
}
}
						...
						<ul class="navbar-nav pull-right ">
							...
							<li class="nav-item dropdown"><a class="smooth-scroll nav-link active" href="#" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Notes</a>
								<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
									...
									<a class="dropdown-item active" href="admin-published-notes.php">Published Notes</a>
...
?>
	...
	<script type="text/javascript">
          function showdata(){
            ...
            $.ajax({
              url:"ajax/admin-published-notes-ajax.php",
              type:"GET",
              data:{
