...
$query_search=mysqli_query($connection,"SELECT users.FirstName,users.LastName,category.Category_name,category.Description,category.CreatedDate,category.IsActive,category.ID FROM category LEFT JOIN users ON category.CreatedBy=users.ID WHERE users.FirstName LIKE '%$search_result%' OR users.LastName LIKE '%$search_result%' OR category.Category_name LIKE '%$search_result%' OR category.Description LIKE '%$search_result%' OR category.CreatedDate LIKE '%$search_result%' OR category.IsActive LIKE '%$search_result%' ORDER BY category.CreatedDate DESC");
if(!$query_search){
	mysqli_error($connection);
}
}
else{
	$query_search=mysqli_query($connection,"SELECT users.FirstName,users.LastName,category.Category_name,category.Description,category.CreatedDate,category.IsActive,category.ID FROM category LEFT JOIN users ON category.CreatedBy=users.ID  ORDER BY category.CreatedDate DESC");
if(!$query_search){
	mysqli_error($connection);
}
}
...
if(isset($_GET['delete_id'])){
	...
	$query_delete=mysqli_query($connection,"UPDATE category SET IsActive=0 WHERE ID=$deleteid");
	if(!$query_delete){
		mysqli_error($connection);
	}
						...
						<ul class="navbar-nav pull-right ">
							...
							<li class="nav-item dropdown"><a class="smooth-scroll nav-link active" href="#" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Settings</a>
								<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
									...
									<a class="dropdown-item active" href="admin-manage-category.php">Manage Category</a>
	...
	<section id="in-progress-notes" class="admin-manage">
		<div class="container">
			...
			<div class="row no-gutters">
				...
				<div class="col-md-6"><button type="submit" class="btn btn-primary btn-admin dashboard-btn" onclick="window.location.href='admin-add-category.php'">ADD category</button></div>
				<form  method="post"><div style="margin-left:auto; margin-right:0px; float:right;" class="dashboard-search">
...
$active="No";
					...
					<td>$fname $lname</td>
					<td>$active</td>
					<td><a href='admin-add-category.php?category_id=$category_id' >
					<img src='images/images/edit.png' ></a>
					<a onclick='Category(this);return false;' href='admin-manage-category.php?delete_id=$category_id'><img src='images/images/delete.png'></a></td>
					";
				}?>
...
var rejectedNotesTable = $('#category-table').DataTable({
    "order": [[ 4, "desc" ]],
