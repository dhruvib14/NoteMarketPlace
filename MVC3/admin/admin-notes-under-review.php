...
$query_approve=mysqli_query($connection,"UPDATE notes SET Status=9,Actioned_By=$admin_id,PublishedDate=NOW() WHERE ID=$approveid");
if(!$query_approve){
	echo mysqli_error($connection);
}
...
if(isset($_GET['reviewid'])){
	$approveid=$_GET['reviewid'];
	$query_approve=mysqli_query($connection,"UPDATE notes SET Status=7,Actioned_By=$admin_id,ModifiedDate=NOW(),ModifiedBy=$admin_id WHERE ID=$approveid");
	if(!$query_approve){
		echo mysqli_error($connection);
	}
...
if(isset($_POST['submit_review'])){
   ...
   $remark=$_POST['remarks'];
	...
	$query_reject=mysqli_query($connection,"UPDATE notes SET Status=10,Actioned_By=$admin_id,ModifiedDate=NOW(),ModifiedBy=$admin_id,Admin_Remarks=$remark WHERE ID=$unpublish_id");
	if(!$query_reject){
		echo mysqli_error($connection);
	}
}	
						...
						<ul class="navbar-nav pull-right ">
							...
							<li class="nav-item dropdown"><a class="smooth-scroll nav-link active" href="#" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Notes</a>
								<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
									<a class="dropdown-item active" href="admin-notes-under-review.php">Notes Under Review</a>
...
<?php
$select_seller=mysqli_query($connection,"SELECT DISTINCT notes.SellerID,users.FirstName FROM notes LEFT JOIN users ON notes.SellerID=users.ID WHERE notes.Status=7 OR notes.Status=8");
if(!$select_seller){
	echo mysqli_error($connection);
}					
?>
	...
	<script type="text/javascript">
          function showdata(){
            ...
            $.ajax({
              url:"ajax/admin-notes-under-review-ajax.php",
              type:"GET",
              data:{
