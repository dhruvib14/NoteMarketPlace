						...
						<ul class="navbar-nav pull-right ">
							...
							<li class="nav-item dropdown"><a class="smooth-scroll nav-link active" href="#" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Notes</a>
								<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
									...
									<a class="dropdown-item active" href="admin-downloaded-notes.php">Downloaded Notes</a>
		...
		<div class="container">
			...
			<div class="row no-gutters">
				<div class="col-md-4 col-lg-2 col-4">
					<div class="container seller-heading">Note</div>
				</div>
				<div class="col-md-4 col-lg-2 col-4">
					<div class="container seller-heading">Seller</div>
				</div>
				<div class="col-md-4 col-lg-2 col-3">
					<div class="container seller-heading">Buyer</div>
				</div>
			</div>
			<?php
$select_type=mysqli_query($connection,"SELECT Category_name,ID FROM category WHERE IsActive=1");
if(!$select_type){
	echo mysqli_error($connection);
}
...
$select_buyer=mysqli_query($connection,"SELECT DISTINCT  downloads.DownloaderID,users.FirstName FROM downloads LEFT JOIN users ON downloads.DownloaderID=users.ID");
if(!$select_buyer){
	echo mysqli_error($connection);
}											
					?>
			<div class="row no-gutters">
				<div class="col-lg-6 col-12">
					...
					<div class="col-lg-4 col-md-4 col-sm-4 col-4 search-type-gap seller">
						<select class="text-hidden form-control options-arrow-down" id="search_type" onchange="showdata()">
							<option selected value="0" disabled>Select type</option>
							<?php while($row=mysqli_fetch_assoc($select_type)){
$category_id=$row['ID'];
...
<option  value='$seller_id' >$seller</option>";
					...
					<div class="col-lg-3 col-md-3 col-sm-3 col-3 search-type-gap seller">
						<select class="text-hidden form-control options-arrow-down" id="search_buyer" onchange="showdata()">
							<option selected disabled value="0">Select Buyer</option>
							<?php while($row=mysqli_fetch_assoc($select_buyer)){
$downloader_id=$row['DownloaderID'];
$downloader=$row['FirstName'];
echo "
<option  value='$downloader_id' >$downloader</option>";
							}?>
						</select>
					</div>
				</div>			
					<div class="dashboard-search ">
						<input type="text" class="form-control" id="search-text-dashboard" placeholder="&#xF002   Search" style="font-family:Sans-serif, FontAwesome" >
						<button type="submit" class="btn btn-primary btn-search dashboard-btn" id="search" onclick="showdata();">Search</button>
				    </div>
			</div>
		</div>
	...
	<script type="text/javascript">
        ...
        function showdata(page_current) {
			...
			let search_buyer = $("#search_buyer").val();
			let search_type = $("#search_type").val();
		  let search_result=$("#search-text-dashboard").val();

            $.ajax({
                url: "ajax/admin-downloaded-notes.php",
                method: "GET",
                data: {
