...
if(isset($_SESSION["useremail"])){
	...
	$institute_name=$_POST['institute_name'];
        ...
        $query_insert_save = "UPDATE notes SET Note_Title='$title',Category=$category,Status=6,
	...
	$institute_name=$_POST['institute_name'];
        ...
        $query_insert_save = "UPDATE notes SET Note_Title='$title',Category=$category,Status=7,
                                  Note_types=$type,Note_Pages=$pages,Description='$description',
                                  University='$institute_name',Country=$country,course='$course_name',
                                  Course_Code='$course_code',Professor_Name='$professor_name',Is_Paid=$sell_type,
                                  price='$price',modifieddate=NOW(), modifiedby=$seller_id 
...
if($result_insert_save)
	{
		
	$query="SELECT SellerID,Note_Title FROM notes where ID=$dashboard_noteid";
		$result= mysqli_query($connection, $query);
...
<?php if(isset($_SESSION['useremail'])){echo "
	...
	<!--header ends-->
	<section id="user-profile"><div class="row">	
<div class="col-12">
	<div class="logo ">
	
		<img src="images/images/banner-home.jpg">
		<h3 id="user-profile-logo" class="text-center">Add Notes</h3>
	</div></div>
</div>
	...
	if(isset($_GET['id'])){
		...
		<div class="row">
			...
			<?php
				...
				<button type="submit" id="user-submit" class="btn btn-primary" onclick='javascript:Publish($(this));return false;' name="publish"  href='addnotes.php?id=".$noteid."'>publish</button><?php }
				else { ?>
